...
object LyricsExtractor {
    fun getLyrics(file: File): String? {
        ...
        val fileending = filename.substring(filename.lastIndexOf('.') + 1, filename.length).toLowerCase(Locale.getDefault())
        var lyric: String? = null
        try {
            when (fileending) {
                "mp3" -> lyric = getLyricsID3(file)
                "mp4", "m4a", "aac" -> lyric = getLyricsMP4(file)
                "ogg", "oga" -> lyric = getLyricsVorbis(file)
            }
        } catch (e: Exception) {
        }
        /* iTunes and some music manager use CR as newline, we replace CRLF and CR to LF to ensure it
         * be correctly displayed */if (lyric != null) {
            lyric = lyric.replace("\r\n", "\n").replace("\r", "\n")
        }
        return lyric
    }
    ...
    private fun readOgg(buf: ByteArray?, `in`: InputStream, bytesinpage: Int, skip: Int): Int {
        ...
        while (toread > 0) {
            if (bytesinpage == 0) {
                ...
                `in`.read(header)
                var count = header[22].toInt() and 0xFF
                while (count-- > 0) {
                    bytesinpage += `in`.read()
                }
            }
    ...
    private fun getLyricsID3(file: File): String? {
        ...
        `in`.read(buffer, 0, 3)
        val ext = buffer[2].toInt() and 32.toByte().toInt() != 0
        `in`.read(buffer)
        var len = buffer[3].toInt() and 0x7F or (buffer[2].toInt() and 0x7F shl 7) or (buffer[1].toInt() and 0x7F shl 14) or (buffer[0].toInt() and 0x7F shl 21)
        if (ext) {
            `in`.read(buffer)
            len -= 4
